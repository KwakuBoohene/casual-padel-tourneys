...
export function estimateTournament(config: TournamentConfig): EstimatedTournament {
  ...
  const roundsFromTarget = config.targetGamesPerPlayer
    ? Math.ceil((playersCount * config.targetGamesPerPlayer) / playersPerRound)
    : Math.ceil((config.tournamentTimeMinutes ?? 90) / matchTimeMinutes(config.pointsPerMatch));
