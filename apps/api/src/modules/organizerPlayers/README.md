...
```text
http/            → /me/players/leaderboard, /me/players/:id, bulk archive/unarchive, rename, merge
...
- Archive releases `nameNormalized` (`archived:{id}`) so a new player can reuse the display name.
  ...
  new identity, moves deltas onto it, and archives both sources. `POST /me/players/archive` and
  `/me/players/unarchive` accept up to 200 ids; already-archived (or already-active) rows are skipped.
  `POST /me/players/:id/rename` changes the career display name; active names stay unique per organizer.
